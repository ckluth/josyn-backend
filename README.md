...
> `JOSYN.SessionBroker` was extracted to its own repo (`josyn-session-broker`) per ADR-025.
...
`SessionLauncher` is the **migration rendezvous**: instead of spawning the old `JobHost.exe`, it spawns
`SessionBroker.exe JOSYN-START @<path>`, which in turn coordinates with the job executable via named-pipe IPC.
...
| `JOSYN.Backend.SessionLauncher` | ✅ Validates job, resolves TechnicalUser, spawns `SessionBroker.exe` |
